...
d'archive TAR. La section~\ref{sec:chiffres} donne quelques chiffres
pour comparer les programmes utilisant \FAceSL avec d'autres
implémentations équivalentes. Enfin, la section~\ref{sec:conclusion}
...
fragiles (terminaison brutale du programme pour des valeurs
...
par l'ANSSI en 2012, utilise donc cette source de données et doit
...
Les fonctions \texttt{parse} travaillent directement sur des chaînes
de caractères alors que \texttt{lwt\_parse} utilisent les flux issus
de la bibliothèque Lwt~\cite{lwt}, une bibliothèque implémentant des
...
décrire des listes d'objets du même type et le type \texttt{container}
...
dès lors qu'il s'agisse d'un type OCaml, et que les fonctions
...
protocole sont des adresses IPv4 et IPv6 encodés directement en
binaires, sur 32 ou 128~bits. Le code suivant montre comment ajouter
...
Le second point consiste à utiliser les mots-clés \texttt{struct} et
...
Afin de présenter le fonctionnement du mot-clé \texttt{struct}, on
...
Empruntant encore à la terminologie du langage C, les unions sont en
...
notion de système autonome (AS), un numéro identifiant un
...
\FAceSL propose d'autres mots-clés, qui ne seront pas décrits en
détails ici:
\begin{itemize}
...
\begin{table}
  \begin{center}
    \begin{tabular}{|r|r|l|}
      ...
      \hline
      512 & \multicolumn{1}{p{4cm}|}{\emph{«~Taille du fichier~»},\newline
        alignée à 512~octets} &
...
caractères, encodant la taille du fichier en octal. Comme cette valeur
...
particulier. Il suffit pour cela de définir un nouveau type et les
fonctions associées\footnote{La fonction \texttt{int\_of\_tarstring}
...
\texttt{tar\_numstring}, c'est-à-dire un entier:
\begin{lstlisting}
struct tar_entry = {
  header : container(512) of tar_header;
...
L'implémentation de \FAceSL comporte
\begin{itemize}
...
\subsection{Formats et protocoles implémentés}

\FAceSL fournit l'implémentation pour les types suivants:
\begin{itemize}
\item les types scalaires de base (entiers, chaînes de caractères);
\item les types liste et conteneur;
\item quelques types utiles (\texttt{magic}, \texttt{ipv4}, etc.);
\item les types ASN.1 de base.
\end{itemize}

\`A partir de cela, les formats et protocoles suivants ont été implémentés:
\begin{itemize}
...
Les deux applications décrites dans les
sections~\ref{sec:tls}~et~\ref{sec:mrt} ont donné lieu à plusieurs
applications:
\begin{itemize}
\item un outil pour extraire les certificats des réponses TLS;
\item un outil de test des versions et suites cryptographiques
  supportées par un serveur TLS donné;
\item un programme pour afficher les annonces de route BGP à partir
  d'un fichier MRT.
\end{itemize}

Pour ces trois applications, nous disposons de plusieurs
implémentations dans différents langages, disponibles sur étagère ou
recodées au sein du laboratoire. Le tableau suivant récapitule le
nombre de lignes et le temps d'exécution pour les différentes
implémentations pour chacune des application:
\begin{tabular}{c|r|r|r|r|}
& \multicolumn{1}{c|}{C} & \multicolumn{1}{c|}{C++} & \multicolumn{1}{c|}{OCaml} & \multicolumn{1}{c|}{OCaml + \FAceSL\footnotemark} \\
\hline
Extraction de certificats & & & & \\
\hline
Test de serveurs TLS & & & & \\
\hline
Affichage des annonces de routes & & & & \\
\hline
\end{tabular}
