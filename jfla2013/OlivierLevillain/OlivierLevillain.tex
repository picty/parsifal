...
d'archive TAR. Enfin, la section~\ref{sec:conclusion} conclut cette
article et présente les perspectives d'évolution.
...
\texttt{union}, \texttt{alias}, \TODO{asn1\_alias}) pour générer automatiquement:
\begin{itemize}
...
sous-chaîne à lire depuis l'entrée. \TODO{Nous l'avons utilisé pour la
...
standard \texttt{ustar} ou non): \lstinline+container(512) of
tar_header+}
...
%\TODO{Ajouter le passage de la RFC}
...
alias\TODO{ comme dans le cas du format TAR.}
...
accepte des types \TODO{enrichis (annotations pour ajouter des vérifications dynamiques)} nommés \FAceSLTypes:
\begin{lstlisting}
...
mode flux, qui repose sur la bibliothèque \texttt{lwt} TODO{Ref: bibliothèque de threads avec une interface de monades}. Pour cela, il
...
\TODO{Dire que la version fonctionne + les commandes pour compiler}
...
%\TODO{Reformuler tout le paragraphe suivant car pas clair}
...
\item description de nouveaux formats de fichiers et de nouveaux
  protocoles (DNS, OCSP) ou formats de fichiers (\texttt{pcap}, PE);
