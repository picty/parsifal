...
\title{\Parsifal, ou comment écrire rapidement des \emph{parsers}
...
constructions apportées par \Parsifal et la section~\ref{sec:tar}
présente un exemple complet d'utilisation de \Parsifal: le format
d'archive TAR. La section~\ref{sec:resultats} donne quelques chiffres
pour comparer les programmes utilisant \Parsifal avec d'autres
...
\emph{parser} sont les suivantes:
\begin{itemize}
\item l'expressivité du langage utilisé (qui peut se quantifier en
  nombre de lignes ou en temps nécessaire pour ajouter le support d'un
  format, d'un protocole ou d'une option). Plus une solution sera
  expressive, plus le développement pourra être rapide;
\item la robustesse de l'outil obtenu (absence d'erreurs de
  segmentation, absence de fuites mémoire, production d'un résultat
  correct);
\item l'efficacité du code produit (vitesse d'exécution, consommation
  mémoire).
\end{itemize}
\noindent
Le ressenti des quatre solutions présentées concernant ces trois
critères est donné dans la table~\ref{tab:props-langages}.

\begin{table}
  \begin{center}
    \begin{tabular}{l|c|c|c|}
                   & Python & C++ & OCaml (DSL) & Parsifal \\
      \hline
      Expressivité &   +    &  -  &      +      &    ++    \\
      \hline
      Robustesse   &   +    &  -- &     +/-     &    ++    \\
    ...
    \end{tabular}
  \end{center}
  \caption{Ressenti des quatre solutions présentées. La robustesse de
    l'implémentation en OCaml (+/-) mérite une petite explication: si
    cette version ne provoquait pas d'erreur de segmentation ou de
    débordement de tampon, la gestion des objets produits était une
    source de fuites mémoire.}
  \label{tab:props-langages}
\end{table}
...
Afin de valider la possibilité d'écrire rapidement avec \Parsifal des
...
décrire en \Parsifal les structures pertinentes pour les études menées
...
L'idée générale de \Parsifal est d'interpréter des descriptions courtes
...
\item des fonctions pour exporter les objets sous forme binaire
  (\texttt{dump\_t});
\item des fonctions pour construire une représentation abstraite de
  l'objet (\texttt{value\_of\_t}).
\end{itemize}
...
Les fonctions \texttt{parse} travaillent sur des chaînes de caractères
alors que \texttt{lwt\_parse} utilisent les flux issus de la
bibliothèque \texttt{Lwt}~\cite{lwt}, une bibliothèque implémentant
des threads synchrones à l'aide d'une interface monadique.

Dans certains cas, les fonctions \texttt{parse}, \texttt{lwt\_parse}
et \texttt{dump} peuvent accepter des paramètres supplémentaires.

Enfin, la fonction \texttt{value\_of} produit une variable de type
\texttt{value} qui peut ensuite être utilisée pour afficher les objets
(\texttt{print\_value} ou \texttt{json\_value}) ou les manipuler à
l'aide d'un langage d'exploration (\texttt{get}). Sans entrer dans
les détails, le type \texttt{value} permet de représenter
symboliquement des valeurs primitives et construites. La définition
(simplifiée) de \texttt{value} est:
\begin{lstlisting}
type value =
  | VBool of bool
  | VSimpleInt of int
...
apportées par \Parsifal, et des exemples permettant de comprendre ce
qu'elles apportent. La grammaire détaillée est donnée en annexe.


\subsection{\'Enumérations}

La première construction ajoutée par \Parsifal est l'énumération, qui
...
val value_of_tls_version : tls_version -> value
\end{lstlisting}
...
Avant de poursuivre avec les constructions apportées par \Parsifal, il
est nécessaire de décrire les \PTypes, les types binaires compris
par \texttt{struct}, \texttt{union} et \texttt{alias}.

\medskip

Tout d'abord, \Parsifal propose des types de base pour lesquels les
implémentations des fonctions \texttt{parse}, \texttt{lwt\_parse},
\texttt{dump} et \texttt{value\_of} sont déjà disponibles: les entiers et
les chaînes de caractères.

En plus de ces types scalaires, le \PType \texttt{list} permet de
...
spécifiée par un champ longueur le ou la précédent. Avec \Parsifal, on
peut spécifier à l'aide de crochets qu'un tel type est attendu:
\texttt{\textbf{list} [uint16] \textbf{of} uint16} permet de décrire
une liste d'entiers sur 16~bits dont la longueur totale en octets est
codées sur un entier 16~bits\footnote{Ce type particulier peut servir
  à décrire la liste des suites cryptographiques (\emph{ciphersuites})
  proposées par un client dans le protocole TLS.}.

\medskip

Enfin, n'importe quel type peut être utilisé comme \PType personnalisé,
dès lors qu'il s'agit d'un type OCaml, et que les fonctions
\texttt{parse}, \texttt{dump} et \texttt{value\_of} correspondantes
...
\item à l'aide des constructions offertes par le préprocesseur (en
  particulier \texttt{struct} et \texttt{union}).
\end{itemize}
...
binaires, respectivement sur 32 et 128~bits. Le code suivant montre
comment ajouter \texttt{ipv4} à l'arsenal des \PTypes
disponibles\footnote{Bien entendu, ce type étant très présent dans les
  protocoles réseau, il est intégré à la bibliothèque de base.}:
\begin{lstlisting}
type ipv4 = string
...
\emph{La définition de \texttt{string\_of\_ipv4} n'est pas strictement
  nécessaire mais elle sert à définir \texttt{value\_of\_ipv4}. En
  effet, pour représenter une adresse IPv4 comme une valeur abstraite,
  on utilise un enregistrement pour y définir une représentation
  imprimable (champ \texttt{@string\_of}\footnote{Les champs
    commençant par un caractère \texttt{@} sont \emph{spéciaux}.}).}


Pour éviter d'avoir à écrire ce genre de code, l'autre méthode
consiste simplement à décrire les nouveaux types comme le montrent les
sections suivantes.


\subsection{Structures}

Les structures que l'on peut décrire avec \Parsifal sont des
enregistrements. Afin de présenter le fonctionnement du mot-clé
\texttt{struct}, on peut s'intéresser à la manière d'implémenter les
messages d'alerte TLS. Ces messages sont spécifiés de la manière
suivante dans TLSv1.2~\cite{rfc5246} (pages 27 et 28):
\begin{verbatim}
...
implémenter cette spécification en \Parsifal, il suffit de créer deux
...
Empruntant encore à la terminologie du langage C, les unions
de \Parsifal sont en réalité des types somme augmentés d'un
constructeur par défaut. L'idée est d'utiliser un discriminant passé
en argument à la fonction \texttt{parse} pour en déduire le
constructeur du type somme à utiliser. Ce discriminant va être comparé
aux différents motifs proposés dans la description de l'union, et le
constructeur adapté sera choisi.
...
notion de système autonome (AS, \emph{Autonomous Systems}), un numéro
identifiant un opérateur. Initialiement, ces AS étaient représentés
sur 16~bits, mais cela n'est plus suffisant désormais et les AS
doivent parfois être décrits sur 32~bits. L'exemple suivant montre
comment décrire un AS avec une union, et comment employer le type
généré dans une structure:
\begin{lstlisting}
...
qui prend un argument supplémentaire (\texttt{as\_size}, annoncé par
l'option \texttt{param}), puis tente de lire le type de segment (un
octet), la longueur du segment (un autre octet), puis une liste
contenant \texttt{path\_segment\_length} éléments du type union
défini. On remarquera que la valeur \texttt{as\_size} est alors passée
comme discriminant à \texttt{parse\_autonomous\_system}.

Voici des extraits du code OCaml généré par \Parsifal à partir des
...
\Parsifal propose d'autres mots-clés et fonctionnalités, qui ne seront
...
\item afin de permettre la description de structures ASN.1 encodées au
  format DER (par exemple les certificats X.509), \Parsifal fournit
  des types prédéfinis et deux mots-clés \texttt{asn1\_alias} et
  \texttt{asn1\_union};
\item parfois, la dissection de messages complexes ne requiert pas une
  ...
  les outils, il est possible de désactiver l'enrichissement de
  manière plus ou moins profonde;
\item il existent certains \PTypes utiles déjà implémentés
  (\texttt{base64\_container}, \texttt{magic}).
\end{itemize}



\section{Exemple d'un développement avec \Parsifal: le format TAR}
\label{sec:tar}
...
\begin{table}
  \begin{center}
    \begin{tabular}{|r|r|l|l|}
      ...
      \hline
      257 &   5	& \multicolumn{1}{c|}{-} & Indicateur \texttt{"ustar"} \\
      \hline
      262 &   3	& \multicolumn{1}{c|}{-} & \emph{Padding} \\
      \hline
...
Afin d'utiliser \Parsifal pour décrire les fichiers au format TAR,
...
\texttt{lwt\_parse\_tar\_file}\footnote{Pour que cette fonction soit
  générée, il faut auparavant ajouter l'option \texttt{with\_lwt} sur
  l'ensemble des types générés.}:
\begin{lstlisting}
...
% ocamlopt -o tar -pp "camlp4o /home/yeye/.ocamlpath/parsifal_syntax/parsifal_syntax.cma" -I /usr/lib/ocaml/lwt -I /home/yeye/.ocamlpath/parsifal_core unix.cmxa lwt.cmxa bigarray.cmxa lwt-unix.cmxa parsifal_core.cmxa tar1.ml
...
Une fois ce nouveau \PType défini, on peut modifier les champs
...
\item des bibliothèques annexes pour implémenter la gestion des
  \PTypes, sur 1300~lignes de code.
\end{itemize}
...
& \multicolumn{1}{c|}{C ou C++} & \multicolumn{1}{c|}{OCaml} & \multicolumn{1}{c|}{OCaml + \Parsifal\footnotemark} \\
\hline
\hline
...
\footnotetext{Pour la colonne «~OCaml + \Parsifal~», le décompte ne
...
Dans tous les cas, on constate que l'implémentation utilisant \Parsifal
...
en OCaml puis avec \Parsifal se sont révélées non seulement plus
...
MRT en C est 33~\% plus rapide que la version reposant sur \Parsifal,
...
outils de dissection de protocoles. Parmi ceux-ci, \Parsifal est une
implémentation générique de \emph{parsers} reposant sur un
pré-processeur \texttt{camlp4} et sur une bibliothèque
auxiliaire. Avec ses~2500~lignes de code, \Parsifal permet de générer
...
Parmi les développements prévus pour \Parsifal, les pistes suivantes
sont envisagées:
\begin{itemize}
...
\item constructions à venir (bitfields, recursifs [utiles même pour du binaire...])
...
\begin{tabbing}
  ...
  \> $\vert$ \> \textbf{\tt list} \textbf{\tt of} \textit{\PType} \\
  \> $\vert$ \> \textbf{\tt list} (\textit{expression}) \textbf{\tt of} \textit{\PType} \\
  \> $\vert$ \> \textbf{\tt list} [\textit{type entier}] \textbf{\tt of} \textit{\PType} \\
  \> $\vert$ \> \textbf{\tt container} (\textit{expression}) \textbf{\tt of} \textit{\PType} \\
  \> $\vert$ \> \textbf{\tt container} [\textit{type entier}] \textbf{\tt of} \textit{\PType} \\
...
\begin{tabbing}
  ...
  \> \> [\textbf{\tt optional}] \textit{identifiant} : \textit{\PType}; \\
  \> \> ... \\
  \> \> [\textbf{\tt optional}] \textit{identifiant} : \textit{\PType} \\
  \> \} \\
  \\
  \textit{union} ::= \textbf{\tt union} \= \textit{identifiant} (\textit{comportement\_union}, [\textit{option list}]) = \\
  \> $\vert$ \textit{motif} $\rightarrow$ \textit{Constructeur} [\textbf{\tt of} \textit{\PType}] \\
  \> ... \\
  \> $\vert$ \textit{motif} $\rightarrow$ \textit{Constructeur} [\textbf{\tt of} \textit{\PType}] \\
  \\
  \textit{comportement\_union} ::\= = \= \textit{Constructor} \\
  \> $\vert$ \> \textit{Constructor} \textbf{\tt of} \textit{\PType} \\
  \\
  \textit{alias} ::= \textbf{\tt alias} \textit{identifiant} = \textit{\PType}
\end{tabbing}
\shorthandon{:}
...
%   \PType pour le constructeur par défaut en le spécifiant dans le
%   comportement.}
%
