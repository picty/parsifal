...
\newcommand{\FAceSLTypes}{BinTypes\xspace}
\newcommand{\FAceSLType}{BinType\xspace}
...
Les \textit{ASN1TypeContents} décrivent le contenu des objets ASN.1
classiques: booléens, entiers, chaînes de bit ou d'octets, OIds,
...
prennent en compte l'en-tête ASN.1 par défaut. De plus, deux fonctions
...
asn1_alias private_rsa_key = (C_Universal, T_Sequence),
...
\item outil pour tester les versions et suites cryptographiques
  supportées par un serveur donné, ou encore pour afficher la partie
  en clair de l'échange: 250~lignes.
\end{itemize}
...
La description des messages MRT nécessite moins de~400~lignes de code,
alors que l'outil permettant d'afficher le contenu disséqué de manière
similaire aux outils existants en comporte~300.
...
travaux exposés ici:
\begin{itemize}
\item \texttt{scapy}, une boîte à outils pour manipuler les paquets
  réseau; ce projet permet également de définir rapidement de nouveaux
  protocoles réseau pour étendre la boîte à outils;
\item \texttt{hachoir}, une bibliothèque pour écrire rapidement des
  dissecteurs pour des formats de fichiers.
\end{itemize}
\noindent Le langage Python semble un candidat naturel pour réaliser
ce genre de projet, de par ses capacités d'introspection. Il est
intéressant de remarquer que l'utilisation d'un pré-processeur en Caml
pour manipuler du Caml permet d'arriver au même résultat.
...
\item automatiser l'animation des protocoles et la génération des
  automates permettant l'étude d'exécutions du protocoles.
\end{itemize}
