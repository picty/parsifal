...
\begin{frame}{Historique (1/2)}
  ...
  \TODO{Eventuellement mieux expliquer les mesures}

  \begin{itemize}
  \item<1-> Au départ, de nombreuses données TLS: 10~campagnes de mesure
    sur les réponses HTTPS du monde IPv4
    \begin{itemize}
    \item 1~campagne menée par Arnaud Ebalard en juillet 2010
    \item 2~campagnes menées par l'EFF en août et décembre 2010
    \item 7~campagnes simultanées (différents stimuli) menées par le
      LRP (Arnaud pour la partie mesures) depuis TSP en juillet 2011
    \item Total: 143~Go de données à traiter\\[.5cm]
    \end{itemize}

  \item<2-> L'objectif était d'en extraire
    \begin{itemize}
...
\begin{frame}{Historique (2/2)}
  ...
  Pour traiter ce volume de données, plusieurs implantations
  \begin{itemize}
  ...
  \item<4-> troisième version en OCaml (avec un DSL, \textit{Domain Specific Language} ressemblant à Python):
    ...
    \end{itemize}
  \item<5-> quatrième version en OCaml, en utilisant un pré-processeur (camlp4):
    \begin{itemize}
    \item \textit{camlp4 est un ensemble d'outils (\emph{parsers},
        pré-processeurs, \emph{pretty printers}) pour OCaml
