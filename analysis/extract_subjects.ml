...
let subject_of_cert accu cert_val =
  try
    let cert = X509Module.pop_object cert_val in
    let s = X509DN.short_display cert.tbs.subject in
    s::accu
  with _ -> accu
      

let extract_subjects msg =
  let (hs_t, hs_msg) = HandshakeModule.pop_object msg.content in
  if msg.content_type = 22 && hs_t = 11
  then List.fold_left subject_of_cert [] (eval_as_list hs_msg)
  else []
...
let _ =
  ...
  try
	...
	try
	  ...
	  List.flatten (List.map extract_subjects tls_msgs)
	with _ -> []
      in
      match tls_moduli with
	| [] -> Printf.printf "%s\t! no certificates\n" name;
	| _ -> iteri (fun i -> fun s -> Printf.printf "%s-%d %s\n" name i s) tls_moduli
    done
  with
    | OutOfBounds s ->
